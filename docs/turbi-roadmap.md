...
- **Estado de Aena en vivo** (FR1526, 26/09/2026: Aena ya había confirmado el aterrizaje y la ficha seguía en «Volando»): con el tramo en curso, la app refresca solo `/flights/AL/N.json` de Render (~0,5 KB, sin ADS-B ni meteorología) al ritmo de sus descargas de Aena (`updated` + 10 min + 20 s; si no hay ciclo nuevo, 60 s y después 120 s), siempre el mismo vuelo físico. Con llegada final, cancelado o desviado, Aena manda: fuera telemetría, radar y distancia animada, y ninguna respuesta tardía del radar la contradice (`js/status-refresh.js`).
- **Vuelo físico ≠ número de vuelo** (CA898, 26/09/2026: GRU → MAD y MAD → PEK con el mismo número y fecha): la app elige el tramo por estado y hora o pregunta por ruta (`chooseLeg`), y pide el radar de ese tramo con `?leg=<physicalFlightKey>`. El servidor lo busca solo entre los tramos de ese número; si no existe o no necesita radar, no aplica (nunca otro tramo). Sin `?leg` y con varios tramos con radar, tampoco escoge. Caché, trabajos, fallos directos y registro de hex van por vuelo físico (la caché por URL, que mezclaba tramos, ya no existe).
- **Trabajo de radar por vuelo** (`server/live.mjs`, `state.radarJobs`: phys → { fase, promesa }): seguimiento por hex, indicativos exactos y arranque de la identificación forman un único trabajo por vuelo físico. Mientras dura, `poll=1` responde `identifying: true` con su fase (`hex`, `direct`, `identify`, `pausa`) y una consulta normal comparte el mismo resultado: dos usuarios nunca lanzan dos búsquedas y un sondeo nunca recibe «sin datos» mientras otro sigue probando indicativos (caso FR4586, 26/09/2026: con 3/min los 4 indicativos tardan más que los 20 s del navegador). El trabajo sigue aunque el navegador abandone. Un 429 en los indicativos exactos deja el trabajo en `pausa` y el servidor lo repite solo al acabar (como mucho 3 veces).
- **Asíncrona:** la respuesta del radar nunca espera a la identificación (`identifying: true`, sin caché). Con el tope de 4 peticiones/min puede tardar 1–2 min: la app vuelve a mirar en segundo plano a los 20, 40, 60, 90, 120, 150 y 180 s (`pollRadar` en `js/radar.js`, reintentos encadenados, sin `setInterval`) con `?poll=1`, que solo lee el estado del trabajo y no llama a adsb.lol. Para en cuanto hay un resultado definitivo, cambia la búsqueda, la ficha deja de verse o se agotan los sondeos. La ficha y Turbulencias nunca esperan.
...
- Un único limitador global para indicativo, zona, hex y trazas. Además de la separación, **como mucho 3 peticiones en cualquier minuto** (la identificación usa como mucho 2 y deja una para el radar; con 4/min seguía llegando algún 429 en Render, cuya IP de salida es compartida): el 25/09/2026, con 5–8 s de separación, adsb.lol dio 8 × 429 en 64 peticiones, siempre tras 6–7 en ~1 min. La identificación tarda más (1–2 min), pero sin pausas de 60 s que dejan a todos sin radar. El radar normal (indicativo y hex) va por delante; una consulta de identificación nunca sale si hay una de radar esperando, y deja al menos 5 s desde la anterior.
- 429 → pausa global adaptativa: el primero respeta Retry-After (o 60 s); si se repite antes de 10 min, 120 s, 180 s… hasta 5 min (nunca menos de Retry-After). 5 aciertos seguidos devuelven la pausa base. El tope por minuto nunca sube solo.
- **Un 429 (o una zona/traza sin respuesta) no es definitivo** (caso FR8297, 26/09/2026): la identificación queda «interrumpida», la respuesta sigue diciendo `identifying: true` con `temporary: true` y `retryAfterSec`, y el propio registro la reanuda al acabar la pausa (como mucho 6 veces). El interés del usuario se mide al programar la pausa: si alguien miró el vuelo en los 5 min anteriores, la reanudación se ejecuta al acabar la pausa aunque dure 5 min y no haya sondeos en medio; para programar otra se vuelve a exigir actividad reciente. `poll=1` nunca arranca trabajo; ninguna consulta arranca una segunda identificación del mismo vuelo. La app muestra «Localizando el avión en el radar…», no «Sin señal». Su sondeo depende del estado: identificando, calendario normal (20…180 s y después cada minuto); en pausa, un único sondeo a `retryAfterSec` + 3 s (sin tope y sin gastar turnos). Como mucho 15 min en total (cubre dos pausas de 5 min). Las zonas se vuelven a pedir al reanudar (las posiciones de hace minutos ya no sirven); las rutas de adsbdb/VRS salen de su caché. Se cancelan en el acto las consultas de identificación pendientes. Durante la pausa: «no disponible» sin llamar a adsb.lol. Se conservan las cachés (radar 60 s por vuelo y por vuelo físico).
