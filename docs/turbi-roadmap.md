...
- **Un 429 (o una zona/traza sin respuesta) no es definitivo** (caso FR8297, 26/09/2026): la identificación queda «interrumpida», la respuesta sigue diciendo `identifying: true` con `temporary: true` y `retryAfterSec`, y el propio registro la reanuda al acabar la pausa (como mucho 6 veces, y solo si alguien ha mirado el vuelo en los últimos 5 min). `poll=1` nunca arranca trabajo; ninguna consulta arranca una segunda identificación del mismo vuelo. La app muestra «Localizando el avión en el radar…», no «Sin señal», y sondea hasta 8 min sin preguntar antes de `retryAfterSec`. Las zonas se vuelven a pedir al reanudar (las posiciones de hace minutos ya no sirven); las rutas de adsbdb/VRS salen de su caché. Se cancelan en el acto las consultas de identificación pendientes. Durante la pausa: «no disponible» sin llamar a adsb.lol. Se conservan las cachés (radar 60 s por vuelo y por vuelo físico).
